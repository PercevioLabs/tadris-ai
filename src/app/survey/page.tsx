    ...
    {options.map((opt) => (
      ...
      >
        ...
        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
          ...
          {value === opt && <div className="w-2.5 h-2.5 bg-indigo-600 rounded-full animate-score-pop" />}
        </div>
        <span className={`text-sm font-medium ${value === opt ? "text-indigo-900" : "text-on-surface-variant"}`}>
          {opt}
        </span>
      </label>
    ))}
    ...
    {options.map((opt) => {
      ...
      return (
        ...
        >
          ...
          <span className={`text-sm font-medium ${isChecked ? "text-indigo-900" : "text-on-surface-variant"}`}>
            {opt}
          </span>
        </label>
      );
...
const ScaleSelect = ({ 
  ...
  <div className="space-y-3 pb-4 last:pb-0 border-b last:border-0 border-outline-variant/20">
    ...
    <div className="flex justify-between max-w-sm px-1">
      <span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-tighter">Not comfortable</span>
      <span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-tighter">Very comfortable</span>
    </div>
  </div>
);
...
const LikertGrid = ({ 
  ...
  <div className="overflow-x-auto -mx-6 px-6 sm:mx-0 sm:px-0">
    <table className="w-full min-w-[500px]">
      <thead>
        <tr>
          <th className="text-left py-4 text-[11px] font-bold text-indigo-900/40 uppercase tracking-widest bg-slate-50/50 px-4 rounded-l-xl">Task</th>
          {options.map(opt => (
            <th key={opt} className="text-center py-4 text-[11px] font-bold text-indigo-900/40 uppercase tracking-widest bg-slate-50/50 px-2 last:rounded-r-xl">
              {opt}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {tasks.map((task) => (
          <tr key={task} className="border-b border-outline-variant/20 last:border-0 hover:bg-slate-50/30 transition-colors">
...
export default function SurveyPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Section 4
    comfortLevels: {
      "Creating exam and quiz questions": 0,
      "Generating lesson plans": 0,
      "Providing written feedback": 0,
      "Answering routine student questions": 0,
      "Converting notes into study materials": 0,
      "Generating documentation/reports": 0
    } as Record<string, number>,
    aiRolePreference: {
      "Creating exam and quiz questions": "",
      ...
      <div className="min-h-screen bg-surface flex items-center justify-center p-6 hero-glow font-body">
        <div className="max-w-md w-full bg-white/70 backdrop-blur-xl border border-white p-12 rounded-[2.5rem] shadow-2xl text-center space-y-8 animate-score-pop">
          ...
          <div className="space-y-4">
            <h1 className="text-3xl font-extrabold font-headline text-on-surface tracking-tight">Thank You!</h1>
            <p className="text-on-surface-variant leading-relaxed">
              Your feedback is invaluable. We appreciate you taking the time to share your academic experience with us.
            </p>
          </div>
          <Link href="/" className="inline-block bg-primary-gradient text-white px-8 py-4 rounded-full font-bold shadow-xl shadow-indigo-500/20 hover:scale-105 transition-all">
            Return to Homepage
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-surface text-on-surface font-body hero-glow pb-20">
      ...
      <nav className="fixed top-0 w-full z-50 bg-glass border-b border-outline-variant/30 px-6 py-4">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <Link href="/" className="text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent font-headline tracking-tight">
            Tadris AI
          </Link>
      ...
      <main className="max-w-3xl mx-auto px-6 pt-32 space-y-12">
        ...
        <header className="space-y-6">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold font-headline leading-tight tracking-tight text-on-surface">
            ...
            <span className="text-transparent bg-clip-text bg-primary-gradient">Shape the Future.</span>
          </h1>
          <div className="bg-white/60 p-6 rounded-3xl border border-white shadow-sm space-y-4">
            ...
            <p className="text-on-surface-variant text-sm leading-relaxed">
              This survey is part of research to understand how university faculty spend their time and where AI tools could meaningfully reduce their workload. Your responses will directly shape which features we prioritize building first.
            </p>
           ...
           <div className="bg-white/40 backdrop-blur-md border border-outline-variant/30 rounded-[2rem] p-8 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-widest px-1">Name (Optional)</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-widest px-1">Email (Optional)</label>
                  <input 
                    type="email" 
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-on-surface-variant uppercase tracking-widest px-1">Institution (Optional)</label>
                <input 
                  type="text" 
        ...
        <section className={`space-y-8 ${step !== 1 ? 'hidden' : ''}`}>
           ...
           <QuestionWrapper title="Q1. Are you currently teaching or lecturing students at a university?" required>
             <RadioGroup 
                name="isTeaching" 
                options={["Yes", "No"]} 
           ...
           <QuestionWrapper title="Q2. What is your primary teaching subject?">
             <RadioGroup 
                name="subject" 
                options={["Arabic Language & Literature", "Computer Science / IT", "Engineering", "English Language", "Natural Sciences (Biology, Chemistry, Physics)", "Business / Management", "Other"]} 
           ...
           <QuestionWrapper title="Q4. What is your average class size?">
             <RadioGroup 
                name="classSize" 
                options={["Fewer than 20 students", "20–40 students", "41–80 students", "More than 80 students"]} 
        ...
        <section className={`space-y-8 ${step !== 2 ? 'hidden' : ''}`}>
           ...
           <QuestionWrapper title="Q6. Approximately how many hours per week do you spend on each of the following activities?" subtitle="Rough estimates are fine.">
             <div className="space-y-4">
                {Object.keys(formData.workloadHours).map((activity) => (
                   <div key={activity} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-xl border border-outline-variant/30 bg-slate-50/50">
                      ...
                      <div className="relative flex items-center">
                        ...
                        />
                        <span className="absolute right-3 text-[10px] font-bold text-on-surface-variant/40 uppercase">Hrs/Wk</span>
                      </div>
                   </div>
                ))}
             </div>
           ...
           <QuestionWrapper title="Q7. Which of the above activities do you find MOST time-consuming or burdensome?" subtitle="Select up to 3.">
             <CheckboxGroup 
                options={["Teaching preparation", "Creating assessments", "Grading and feedback", "Responding to queries", "Course documentation and reporting"]} 
           ...
           <QuestionWrapper title="Q8. When during the semester does your workload feel heaviest?">
             <RadioGroup 
                ...
                options={["Start of semester (setup, planning, registration)", "Mid-semester (exams, assignments)", "End of semester (final exams, reports, grade submission)", "Consistently throughout the semester"]} 
           ...
           <div className="flex gap-4">
             <button onClick={() => setStep(1)} className="flex-1 bg-white border border-indigo-100 text-indigo-600 py-4 rounded-full font-bold transition-all hover:bg-slate-50 flex items-center justify-center gap-2">
               <span className="material-symbols-outlined">arrow_back</span>
               Back
             </button>
             <button onClick={() => setStep(3)} className="flex-[2] bg-primary-gradient text-white py-4 rounded-full font-bold shadow-xl shadow-indigo-600/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
        ...
        <section className={`space-y-8 ${step !== 3 ? 'hidden' : ''}`}>
           ...
           <QuestionWrapper title="Q9. Which general tools do you currently use for teaching and course management?" subtitle="Select all that apply.">
              <CheckboxGroup 
                options={["LMS (Blackboard, Moodle, Canvas, etc.)", "Microsoft Word / Google Docs", "Microsoft PowerPoint / Google Slides", "Microsoft Excel / Google Sheets", "Email only", "Other"]} 
           ...
           <QuestionWrapper title="Q10. Which AI tools have you tried or currently use for teaching purposes?" subtitle="Select all that apply.">
              <CheckboxGroup 
                options={["ChatGPT", "Google Gemini", "Microsoft Copilot", "Grammarly or similar AI writing tools", "Other AI tool", "I do not use any AI tools"]} 
           ...
           <QuestionWrapper title="Q11. How often do you use AI tools for teaching-related work?">
              <RadioGroup 
                name="aiFrequency" 
                options={["I have never tried AI tools", "I tried once or twice but stopped", "Occasionally (a few times a month)", "Regularly (weekly)", "Daily"]} 
                value={formData.aiFrequency}
                onChange={(v) => updateField("aiFrequency", v)}
              />
           </QuestionWrapper>

           {(!formData.aiTools.includes("I do not use any AI tools") && formData.aiFrequency !== "I have never tried AI tools") && (
             <QuestionWrapper title="Q12. What do you mainly use AI tools for?" subtitle="Select all that apply.">
                <CheckboxGroup 
                  options={["Drafting lesson content or slides", "Creating exam or quiz questions", "Summarizing or simplifying material", "Writing or editing documents", "Getting feedback on academic writing", "Answering student questions", "Other"]} 
           ...
           <div className="flex gap-4">
             <button onClick={() => setStep(2)} className="flex-1 bg-white border border-indigo-100 text-indigo-600 py-4 rounded-full font-bold transition-all hover:bg-slate-50 flex items-center justify-center gap-2">
               <span className="material-symbols-outlined">arrow_back</span>
               Back
             </button>
             <button onClick={() => setStep(4)} className="flex-[2] bg-primary-gradient text-white py-4 rounded-full font-bold shadow-xl shadow-indigo-600/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
        ...
        <section className={`space-y-8 ${step !== 4 ? 'hidden' : ''}`}>
           <div className="flex items-center gap-3 mb-2">
             <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-bold text-sm">4</div>
             <h2 className="text-2xl font-bold font-headline text-on-surface">Your Attitude Towards AI</h2>
           </div>

           <QuestionWrapper title="Q13. How comfortable would you be using AI for each of the following tasks?" subtitle="1 = Not comfortable at all · 5 = Very comfortable">
           ...
           <QuestionWrapper title="Q14. For each task, do you think AI should play a role in it?">
              <LikertGrid 
                tasks={[
                  "Creating exam and quiz questions",
                  "Generating lesson plans and teaching materials",
                  "Providing written feedback on student essays",
                  "Answering routine student questions outside class hours",
                  "Converting lecture notes or recordings into study materials",
                  "Generating course documentation and institutional reports"
                ]}
           ...
           <QuestionWrapper title="Q15a. What are your biggest concerns about using AI tools for teaching?">
              <CheckboxGroup 
                options={[
           ...
           <QuestionWrapper title="Q15b. For you to confidently use an AI teaching tool, which would need to be true?">
              <CheckboxGroup 
                options={[
           ...
           <div className="flex gap-4">
             <button onClick={() => setStep(3)} className="flex-1 bg-white border border-indigo-100 text-indigo-600 py-4 rounded-full font-bold transition-all hover:bg-slate-50 flex items-center justify-center gap-2">
               <span className="material-symbols-outlined">arrow_back</span>
               Back
             </button>
             <button onClick={() => setStep(5)} className="flex-[2] bg-primary-gradient text-white py-4 rounded-full font-bold shadow-xl shadow-indigo-600/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
               Final Section
               <span className="material-symbols-outlined">arrow_forward</span>
             </button>
           </div>
        ...
        <section className={`space-y-8 ${step !== 5 ? 'hidden' : ''}`}>
           <div className="flex items-center gap-3 mb-2">
             <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-bold text-sm">5</div>
             <h2 className="text-2xl font-bold font-headline text-on-surface">Follow-Up</h2>
           </div>

           <QuestionWrapper title="Q16. Would you be willing to join a 30-minute follow-up interview to share more about your teaching experience?">
              <RadioGroup 
                name="followUp" 
                options={["Yes", "No"]} 
              ...
              {formData.followUp === "Yes" && (
                <div className="mt-6 p-6 bg-indigo-50/50 rounded-2xl border border-indigo-100 space-y-4 animate-slide-up-fade">
                  <p className="text-xs font-bold text-indigo-900/60 uppercase tracking-widest">Please provide contact details if not added above:</p>
                  <input 
                    type="text" 
           ...
           <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white space-y-4 shadow-2xl shadow-indigo-200">
             ...
             <p className="text-slate-400 text-sm leading-relaxed">
               By clicking submit, your responses will be used to help us build tools that truly support educators. Thank you for your contribution to the future of teaching.
             </p>
             <div className="flex gap-4 pt-4">
               <button onClick={() => setStep(4)} className="flex-1 bg-white/10 hover:bg-white/20 text-white py-4 rounded-full font-bold transition-all flex items-center justify-center gap-2">
                 <span className="material-symbols-outlined">arrow_back</span>
                 Review
               </button>
               <button 
